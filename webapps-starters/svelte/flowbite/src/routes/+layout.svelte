<script>
    ...
    LoadedComponents = false
		const response = await fetch(
			`/components/list?framework=svelte&components=flowbite&icons=lucide`
		);
...
<div class="min-h-screen min-h-screen overflow-y-auto antialiased">
	...
	<div class="fixed bottom-0 left-0 w-screen max-w-screen
		...
		<a on:click={()=>{ generate_mode = 'description' }} id="mode_json" class="hidden cursor-pointer sm:block md:-ml-10">
